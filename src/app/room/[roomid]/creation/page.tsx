...
export default function CharacterCreationPage() {
  ...
  const handleCreateCharacter = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        const imageRef = ref(storage, `rooms/${roomId}/characters/${character.Nomperso}-${userId}-image`)
      ...
      const characterData = {
        ...character,
        imageURL,
        userId, // Add userId to identify the owner
        roomId, // Add roomId for reference
        type: 'joueurs',
      ...
      // Save to room's character collection
      await addDoc(collection(db, `rooms/${roomId}/characters`), characterData)
  ...
  const renderImageSelection = useCallback(() => (
    <div className="space-y-4">
      <div className="text-center">
        <h3 className="text-lg font-semibold mb-4">Image du personnage (optionnel)</h3>
      </div>
      <Input
        type="file"
        accept="image/*"
      ...
      <div className="flex justify-between mt-6">
        ...
        <Button 
          ...
          disabled={!character.Nomperso || !character.Nomjoueur}
        >
          Créer le personnage
        </Button>
      </div>
    </div>
  ), [imagePreview, character.Nomperso, character.Nomjoueur])
  ...
  return (
    <div className="container mx-auto p-4">
      <Card className="w-full max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>Création de personnage - Salle {roomId}</CardTitle>
        </CardHeader>
        <CardContent>
